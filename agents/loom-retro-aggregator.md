...
- **Learned guidance writer (M0.11 から)**: 承認 finding のうち `target_artifact == "agent-prompt"` を持つものを `<project>/.claude-loom/project-prefs.json` の `agents.<target>.learned_guidance[]` に Edit / Write で書き込む（user 昇格 opt-in 時は `~/.claude-loom/user-prefs.json`）
...
retro session 終了時（action plan 確定後の最終 step）に以下 auto-prune logic を実行：

### ttl_sessions main（決定論的 auto-deactivate）

```
active learned_guidance を全 scan:
  retro_session_distance = current_retro_session_index - first_retro_session_index_after_added_at
  (retro_session_history から計算)
  ttl_sessions != null && retro_session_distance >= ttl_sessions
    → active: false に自動 deactivate
  ttl_sessions == null (default) → infinite、auto-deactivate なし
```

### last_used_in update（lens 参照時、central write）

- 各 lens が dispatch 時に `[loom-learned-guidance]` block で guidance injection を受け取る
- aggregator が retro session 終了時に **参照された guidance の `last_used_in: <current_retro_id>` を更新**（lens は read のみ、aggregator が write 中央集権）
- 参照確認方法: lens output JSON に `applied_guidance_ids: []` field を要求 or lens が参照した guidance id を report

### meta lens stale guidance audit（proposal、user 主導）

- `current_retro_id` から遡って `last_used_in` が N session 連続未使用（initial threshold = 3）→ meta-axis finding 化候補
- finding 経由で user 承認後 deactivate（auto deactivate じゃない、meta lens 経由で構造的判断 gate を設ける）
- `last_used_in: null` 初期値 guidance は `added_at` からの session 距離で stale 判定

**責務分離**: auto-deactivate（ttl 決定論的）と user 承認 prune（last_used_in 経由 meta lens）を明確に分離。SPEC §6.9.4.5 参照。
