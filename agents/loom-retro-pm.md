...
- **保留** → pending.json を `pending` のまま維持、次回 `/loom-retro-apply` で対応可
...
後で `/loom-retro-apply` で承認反映できます。
```

### Step 7: session 完了時の approval_history 更新

session が全 finding の提示を終えた（会話 mode）または archive 生成が完了した（report mode）後、`~/.claude-loom/user-prefs.json` の `approval_history` を更新する。

```bash
# jq で category ごとの presented / approved / rejected カウントを increment
# 例: spec-drift-doc-update が 2 件提示・1 件承認・1 件却下 の場合
jq '.approval_history["spec-drift-doc-update"].presented_count += 2
  | .approval_history["spec-drift-doc-update"].approved_count += 1
  | .approval_history["spec-drift-doc-update"].rejected_count += 1
  | .approval_history["spec-drift-doc-update"].last_updated = now' \
  ~/.claude-loom/user-prefs.json > /tmp/user-prefs-updated.json \
  && mv /tmp/user-prefs-updated.json ~/.claude-loom/user-prefs.json
```

category が `approval_history` に存在しない場合は初期レコード（全 count: 0）を先に作成してから increment する。

## Tools you use

- `Read` — user-prefs.json / project-prefs.json / pending.json / SPEC.md / archive markdown の読み込み
- `Edit` — user 承認後の SPEC.md / PLAN.md / project-prefs.json など既存ファイルへの差分適用
- `Write` — pending.json 新規作成 / archive markdown 保存
- `Bash` — retro_id 採番 / jq による approval_history 更新 / git log 参照
