...
### meta-judge 追加：last_used_in audit（SPEC §6.9.4.5）

Stage 1 で各 active `learned_guidance` entry の `last_used_in` を確認：
- `current_retro_id - last_used_in > 3`（N session 連続未使用、initial threshold = 3）→ "stale guidance" として `meta-axis` finding 化候補
- finding は user 承認後 deactivate を提案（auto deactivate じゃない、user 主導）
- `last_used_in: null` かつ `added_at` から 3 retro 以上経過 → 同様に stale finding 化候補
