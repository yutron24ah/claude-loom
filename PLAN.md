...
- [ ] PLAN.md M1 milestone 拡張（本タスク） <!-- id: m1-t1 status: todo -->
- [ ] pnpm workspace root 初期化（package.json + pnpm-workspace.yaml + tsconfig 共通設定 + .gitignore for node_modules） <!-- id: m1-t2 status: todo -->
- [ ] daemon/ package init（package.json、TypeScript config、依存：fastify / @fastify/websocket / @trpc/server / zod / drizzle-orm / better-sqlite3 / nanoid / vitest / tsx） <!-- id: m1-t3 status: todo -->
- [ ] daemon/src/db/schema.ts: Drizzle で SPEC §6.1 全 11 table 定義（projects / events / sessions / subagents / agent_pool / tasks / token_usage / notes / plan_items / spec_changes / consistency_findings） <!-- id: m1-t4 status: todo -->
- [ ] drizzle-kit generate で initial migration SQL 出力 + DB client wrapper 実装 <!-- id: m1-t5 status: todo -->
- [ ] daemon/src/server.ts: Fastify + tRPC adapter setup、127.0.0.1:5757 bind、health endpoint <!-- id: m1-t6 status: todo -->
- [ ] daemon/src/router.ts: tRPC AppRouter root（project / session / agent / plan / consistency / approval / note / config / events sub-routers） <!-- id: m1-t7 status: todo -->
- [ ] daemon/src/routes/: 8 sub-router 実装（project / session / agent / plan / consistency / approval / note / config）、各 zod input/output schema <!-- id: m1-t8 status: todo -->
- [ ] daemon/src/routes/events.ts: tRPC subscription procedures（onAgentChange / onPlanChange / onFindingNew / onApprovalRequest 等） <!-- id: m1-t9 status: todo -->
- [ ] daemon/src/events/broadcaster.ts: WS event hub（subscriptions に push する dispatch 機構） <!-- id: m1-t10 status: todo -->
- [ ] daemon/src/hooks/ingest.ts: POST /event handler（bash hook ingestion、tRPC 通さん plain HTTP）+ events table 永続化 <!-- id: m1-t11 status: todo -->
- [ ] daemon/src/hooks/correlation.ts: Subagent 相関 FIFO ロジック（SPEC §6.4 既存仕様） <!-- id: m1-t12 status: todo -->
- [ ] daemon/src/project/detect.ts: project 判定（git root + .claude-loom/project.json marker、SPEC §6.7） <!-- id: m1-t13 status: todo -->
- [ ] daemon/src/security/token.ts: nanoid 生成 + ~/.claude-loom/daemon-token (chmod 600) read/write、tRPC middleware で headers verify <!-- id: m1-t14 status: todo -->
- [ ] daemon/src/lifecycle/idle-shutdown.ts: 30 分 inactivity で auto-shutdown（最終 event timestamp 監視） <!-- id: m1-t15 status: todo -->
- [ ] daemon/src/lifecycle/event-cleanup.ts: events rolling delete (30 日 OR 200MB)、daily loop <!-- id: m1-t16 status: todo -->
- [ ] daemon/src/config.ts: ~/.claude-loom/config.json read/write（default 生成）、SPEC §6.10 schema <!-- id: m1-t17 status: todo -->
- [ ] hooks/ 5 bash script 新設（session_start / pre_tool / post_tool / stop / SubagentStop、curl POST /event） <!-- id: m1-t18 status: todo -->
- [ ] commands/loom.md / loom-status.md / loom-stop.md 新設（slash commands、daemon 起動 / 状態 / 停止） <!-- id: m1-t19 status: todo -->
- [ ] install.sh 拡張：settings.json に hooks 配線（jq + atomic mv）+ daemon 関連 symlink 必要なら追加 <!-- id: m1-t20 status: todo -->
- [ ] daemon/test/: Vitest 設定 + 各 router の unit test + ingestion / lifecycle integration test <!-- id: m1-t21 status: todo -->
- [ ] tests/REQUIREMENTS.md REQ-028 追加 + tests/daemon_test.sh 新設（harness 側 smoke test：daemon start / health / hook POST） <!-- id: m1-t22 status: todo -->
- [ ] README.md + CLAUDE.md に daemon 起動方法 + tRPC AppRouter import 例追記 <!-- id: m1-t23 status: todo -->
- [ ] docs/DOC_CONSISTENCY_CHECKLIST.md M1 check items 追加 <!-- id: m1-t24 status: todo -->
- [ ] 全 test PASS + tag m1-complete + main merge <!-- id: m1-t25 status: todo -->
