...
- [ ] PLAN.md M0.11.1 milestone 挿入（本タスク） <!-- id: m0.11.1-t1 status: todo -->
- [ ] tests/REQUIREMENTS.md REQ-035 追加 <!-- id: m0.11.1-t2 status: todo -->
- [ ] SPEC §6.9.x で pending.json schema 完全定義（zod、`applied_in` field 含む、M2.1 verdict_evidence.json schema 化と同パターン） <!-- id: m0.11.1-t3 status: todo -->
- [ ] SPEC §6.9.4 で learned_guidance schema 拡張（`last_used_in` field 追加 + auto-prune rule 明文化） <!-- id: m0.11.1-t4 status: todo -->
- [ ] templates/{user,project}-prefs.json.template に new field example 追加 <!-- id: m0.11.1-t5 status: todo -->
- [ ] 既存 pending.json 群（4 retro 分）に `applied_in` field 後付け migration script <!-- id: m0.11.1-t6 status: todo -->
- [ ] agents/loom-retro-pm.md Stage 0 拡張（過去 pending.json scan + `applied_summary.json` 生成、verdict_evidence.json と同パターン） <!-- id: m0.11.1-t7 status: todo -->
- [ ] 4 lens prompt に applied summary context 注入 mechanism 追記（pj-judge/process-judge/meta-judge/researcher） <!-- id: m0.11.1-t8 status: todo -->
- [ ] agents/loom-retro-counter-arguer.md の stale finding detection section を **rollback**（symptomatic patch を構造的解決後に消滅、proc-NEW-1 rollback discipline 初例） <!-- id: m0.11.1-t9 status: todo -->
- [ ] agents/loom-retro-aggregator.md の learned_guidance write logic に auto-prune 拡張（use_count threshold + ttl_sessions expiry 自動 active false 化） <!-- id: m0.11.1-t10 status: todo -->
- [ ] tests/{prefs,retro,agents}_test.sh 拡張（applied_in schema + applied_summary build + auto-prune assertion） <!-- id: m0.11.1-t11 status: todo -->
- [ ] docs/RETRO_GUIDE.md に lifecycle tracking section 追加（applied_in 規約 + auto-prune rule + symptomatic rollback 例） <!-- id: m0.11.1-t12 status: todo -->
- [ ] docs/DOC_CONSISTENCY_CHECKLIST.md M0.11.1 check items 追加 <!-- id: m0.11.1-t13 status: todo -->
- [ ] 全 test PASS + tag m0.11.1-complete + main merge <!-- id: m0.11.1-t14 status: todo -->

**M0.11.1 完成基準**：
- `./tests/run_tests.sh` 全 PASS 維持（既存 12 + applied_in / applied_summary / auto-prune assertion 拡張）
- SPEC §6.9.x に pending.json zod schema 完全定義 + `applied_in` field
- SPEC §6.9.4 に learned_guidance auto-prune rule + `last_used_in` field
- agents/loom-retro-pm.md Stage 0 で過去 pending.json scan + applied_summary.json 生成 mechanism
- 4 lens prompt が applied summary を context として参照
- agents/loom-retro-counter-arguer.md の stale finding detection section **rollback 完了**（symptomatic patch を structural 解決で消滅、SPEC §3.9.x P4 理想形の初例として archive）
- agents/loom-retro-aggregator.md に auto-prune logic
- 既存 4 retro session の pending.json に `applied_in` field 後付け migration 完了
