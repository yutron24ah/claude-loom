...
- [ ] templates/{user,project}-prefs.json.template に `last_used_in` field example 追加 <!-- id: m0.11.1-t6 status: todo -->
- [ ] migration script: 既存 4 retro session pending.json (2026-04-29-001 / 2026-05-02-001 / 2026-05-02-002 + 古い 1 件確認) の `applied_in` + `apply_history` field 後付け書き込み + schema_version 1 → 2 migrate <!-- id: m0.11.1-t7 status: todo -->
- [ ] agents/loom-retro-pm.md Stage 0 拡張 (applied_summary.json lazy build 5 step、SPEC §6.9.7 手順) <!-- id: m0.11.1-t8 status: todo -->
- [ ] 4 lens prompt (pj-judge/process-judge/meta-judge/researcher) に `applied_summary_path` injection + `Read` tool 参照 mechanism 追記 <!-- id: m0.11.1-t9 status: todo -->
- [ ] agents/loom-retro-aggregator.md に learned_guidance auto-prune logic (`ttl_sessions` main auto-deactivate + `last_used_in` update on retro reference) 追加 <!-- id: m0.11.1-t10 status: todo -->
- [ ] applied_summary mechanism dry-run test: 既存 4 retro session pending.json で applied_summary build → schema validate → fixture diff (t11 rollback 前安全網) <!-- id: m0.11.1-t11 status: todo -->
- [ ] agents/loom-retro-counter-arguer.md の stale finding detection section **物理削除** (M3.0 retro proc-NEW-1 構造的消滅、SPEC §3.9.x P4 理想形初例、t11 dry-run test pass 後実施) <!-- id: m0.11.1-t12 status: todo -->
- [ ] tests/{prefs,retro,agents}_test.sh 拡張 (`applied_in` schema + `applied_summary` build + auto-prune assertion + counter-arguer stale check section **不在** assertion) <!-- id: m0.11.1-t13 status: todo -->
- [ ] 全 test PASS (`./tests/run_tests.sh` 12 PASS 維持) + tag m0.11.1-complete + main merge <!-- id: m0.11.1-t14 status: todo -->

**M0.11.1 完成基準**：
- `./tests/run_tests.sh` で **12 PASS** 維持（既存 12 test files + applied_in / applied_summary / auto-prune / counter-arguer stale check 不在 assertion 拡張）
