...
- [x] LearnedGuidance scope filter pill 4 値 (all / Agents (3) / loom-review / loom-retro) + `keyKind` badge + `keyPath` 表示、aggregator `WRITE` badge、GD-SCOPE-* prefix Vitest case 3+ <!-- id: m0.18-t4 status: done committed_sha: 808f1b6 (impl in worktree-agent-aec7b928fc7524cb7) + 251e005 (merge to feat/m0.18-... via --no-ff ort strategy、conflict 0 auto-resolved) path: C (loom-developer self-review、Task tool deferred、4 観点 self-checklist 全 PASS、TDD red→green 確認 tdd_red_confirmed: true (RED evidence: 18/18 failed)、GD-SCOPE-* prefix 18 cases (要件 3+ 大幅クリア) + worktree base 975/975 (post-merge 1109/1109 全 ui test PASS regression なし)、tsc new error 0、worktree base m0.17 91a947e (M0.18 全 work を含まん古い base、ort auto-merge で REQUIREMENTS.md cleanly resolved)、REQ-144..149 採番、handoff_required: false、worktree dispatch 正規 path (worktree 内 commit → PM merge --no-ff)、formal reviewer follow-up は Phase 4 closure でまとめ) planned_files: ui/src/views/guidance/LearnedGuidanceView.tsx, ui/src/styles/screens/guidance.css, ui/test/views/guidance/guidance-scope.test.tsx, tests/REQUIREMENTS.md -->

- [x] SessionList `reviewer_agent` → `reviewer (skill)` column rename + 値 skill identifier reformat (`loom-review/trio.code` 等)、projection 関数で DB 互換性吸収、SES-LBL-* prefix Vitest case 3+ <!-- id: m0.18-t5 status: done committed_sha: 11b0ca1 path: C (loom-developer self-review、Task tool deferred、4 観点 self-checklist 全 PASS、TDD red→green 確認 tdd_red_confirmed: true、SES-LBL-* prefix 9 cases (要件 3+ クリア) + post-merge 1109/1109 全 ui test PASS regression なし、tsc new error 0、REQ-150..155 採番、handoff_required: false、retro candidate F-proc-NEW2-RE: worktree dispatch (worktree-agent-ac7dac9f53ffda5b5) やのに dev が main branch (feat/m0.18-...) 直 commit、worktree HEAD は base 91a947e のまま放置 — t1 と同じ F-proc-NEW pattern 再発、Agent tool isolation worktree の commit destination 不確定挙動 = 2 回目の観測 → retro F-proc-NEW2 として Phase 4 closure retro 必須 escalation、formal reviewer follow-up は Phase 4 closure でまとめ、SessionItem.reviewer_agent? optional field を redesign/api/types.ts に追加 (REQ-079 frozen list 対象外、安全)、REVIEWER_AGENT_SKILL_MAP の mapping pair: loom-reviewer→loom-review/single, loom-code-reviewer→loom-review/trio.code, loom-security-reviewer→loom-review/trio.security, loom-test-reviewer→loom-review/trio.test) planned_files: redesign/api/types.ts, ui/src/views/session-list/SessionListView.tsx, ui/test/views/session-list/ses-lbl.test.tsx, tests/REQUIREMENTS.md -->
...
- [x] TokenMeter `useTokenUsage({ enabled: !!session.active })` gate、session 起動前は polling 抑止 <!-- id: m0.18-t6 status: done committed_sha: c5a0266 path: C (loom-developer self-review、Task tool deferred、4 観点 self-checklist 全 PASS、TDD red→green 確認 tdd_red_confirmed: true (5/5 fail captured)、TM-GATE-* prefix 5 cases (要件 3+ クリア) + ui 1114/1114 PASS regression なし、tsc new error 0、REQ-156..158 採番、handoff_required: false、Option A (caller-driven) 採用 = TokenMeterView が useScenario().key === ACTIVE_SCENARIO_KEY で sessionActive 判定、ACTIVE_SCENARIO_KEY typed const を useTokenUsage.ts から export して文字列直接比較禁止規律 (SPEC §3.6.10) 準拠、既存 isActive = isConnected && externalEnabled ロジック活用、retro candidate F-USER-NEW: 'live' ScenarioKey semantics 検討余地 — 現仕様では 'live' も polling 抑止、real daemon WS 接続意味する場合 gate 条件 (key === ACTIVE || key === 'live') 拡張要、PLAN.md SSoT は session.active のみ指定で本 task scope 外、formal reviewer follow-up は Phase 4 closure でまとめ) planned_files: ui/src/live/useTokenUsage.ts, ui/src/views/tokens/TokenMeterView.tsx, ui/test/live/use-token-usage.test.tsx, tests/REQUIREMENTS.md -->

- [x] approval.decide NOT_FOUND TRPCError onError catch + toastBus.push error + retry action、ERR-NF-* prefix Vitest case 3+ <!-- id: m0.18-t7 status: done committed_sha: cb2b416 path: C (loom-developer self-review、Task tool deferred、4 観点 self-checklist 全 PASS、TDD red→green 確認 tdd_red_confirmed: true (RED evidence: import resolve error)、ERR-NF-* prefix 4 cases (ERR-NF-01/02 × 2/03、要件 3+ クリア) + ui 1118/1118 PASS regression なし、tsc new error 0、REQ-159..161 採番、handoff_required: false、新規 useApprovalMutations hook 設置、TRPC_CODES typed const で文字列直接比較禁止規律 (SPEC §3.6.10) 準拠、retryLastDecide() で lastInputRef payload 再送、toastBus に Toast.action?: ToastAction field + approval_not_found event + emitApprovalNotFound helper 追加、retro candidate F-proc-NEW: simulateDecideError test-only escape hatch pattern (tRPC mutation callbacks が Vitest で real WS なしでは trigger できん制約への対応、hook return type 含めて SRP + co-located で対応)、test gap (low severity): decide() 直接呼出し test なし — 次 milestone で拾う候補、formal reviewer follow-up は Phase 4 closure でまとめ) planned_files: ui/src/notifications/toastBus.ts, ui/src/live/useApprovalMutations.ts, ui/test/notifications/not-found-toast.test.tsx, tests/REQUIREMENTS.md -->

- [x] Sidebar.tsx 残存有無確認、AppShell Drawer + TopBar で完全置換済なら削除、import 跡 grep clean <!-- id: m0.18-t8 status: done committed_sha: (no-op, PM direct close) path: D (PM direct status confirmation、dev dispatch 不要 no-op task)、PM audit 結果: `ls ui/src/routing/Sidebar.tsx` → No such file or directory (既に削除済)、`grep -rn "Sidebar" ui/src/` → import reference 0 件 (clean)、`grep -rn "Sidebar" ui/test/` → 2 件 historical comment のみ: (1) `ui/test/AppShell.test.tsx:231-232` Drawer 移行 historical note "AppShell redesign replaces Sidebar with Drawer", (2) `ui/test/notifications/not-found-toast.test.tsx:14` t7 dev が PM prompt から拾った t8 mention (実 code reference なし、harmless)、Sidebar.tsx は M0.17 (UI Redesign Port Correction) で既に AppShell Drawer + TopBar に完全置換済、本 task は単なる status check + no-op confirmation、dispatch せず PM が grep audit で close (cost 節約) planned_files: ui/src/routing/Sidebar.tsx (既に削除済、null op) -->

### Phase 4: doc + QA integration + closure

- [x] QA Test Matrix prefix 全 7 種 (SPIRIT/CUSTOM-TREE/RETRO-LC/RETRO-ADM/GD-SCOPE/SES-LBL/ERR-NF) を Vitest + Playwright case として実装 verify、Playwright baseline regenerate (3 view 新 visual snapshot)、`pnpm --filter @claude-loom/ui test` + `pnpm --filter @claude-loom/ui exec playwright test` 全 PASS <!-- id: m0.18-t9 status: done committed_sha: c7d96bb path: C (loom-developer self-review、Task tool deferred、4 観点 self-checklist 全 PASS、verify-only task)、Vitest 1118/1118 unchanged (verify only、test 数変動なし SSoT 保証)、Playwright 43/43 PASS (24 new m0.18-* + 19 existing) + 3 new e2e spec (m0.18-room-spirit / m0.18-customization-tree / m0.18-retro-kpt) + 9 darwin PNG baseline (room-spirit-hybrid-idle/active/agent-detail + customization-tree-initial/agent-editor/skill-editor/aggregator-write + retro-kpt-initial/admin-expanded)、tsc new error 0、m0.15_baseline_status: untouched (Option β 採用、historical 温存 + m0.18 新世代併存)、req_ids_added: none (verify-only task、既存 REQ-115..161 SSoT)、handoff_required: false、self-review findings 2 件 low severity non-blocking: (1) coverage comment inaccuracy (room-spirit.spec.ts:12、"7 screenshots + 3 interaction" claim だが actual 3 screenshots + 6 DOM/interaction), (2) dismissToasts helper duplication 3 files (Playwright shared helpers 制約)、retro candidates 3 件 Phase 4 closure retro 評価候補: (a) F-USER-NEW: pma-overlay pointer-event interception pattern — mock=active で PMChatPanel が click 全 blockする gotcha、mock=idle 推奨 pattern を helpers 化候補, (b) F-proc-NEW: Playwright `--update-snapshots missing` 挙動 — 1st run で snapshot write でも fail report、`--update-snapshots all` で stabilize 必要, (c) F-USER-NEW: e2e helpers DRY (dismissToasts 3 file duplication) — `ui/e2e/helpers.ts` 抽出 Phase 3 architectural debt 候補、rpg/office spiritMode e2e gap (unit test 補完済) は acceptable scope、formal reviewer follow-up は Phase 4 closure でまとめ) planned_files: ui/e2e/m0.18-room-spirit.spec.ts, ui/e2e/m0.18-customization-tree.spec.ts, ui/e2e/m0.18-retro-kpt.spec.ts, ui/e2e/__screenshots__/m0.18-*.png (9 baselines) -->

- [x] `docs/SCREEN_REQUIREMENTS.md` 各 view 更新 (customization tree / retro KPT+lifecycle / room spirit summoning)、`docs/DOC_CONSISTENCY_CHECKLIST.md` M0.18 check items 追加、`SPEC.md §15 Topic Index` 修正済 (本 milestone) verify <!-- id: m0.18-t10 status: done committed_sha: f83b752 path: C (loom-developer self-review、Task tool deferred、4 観点 self-checklist 全 PASS、docs-only task)、SCREEN_REQUIREMENTS.md +92 行 (§3.7 KPT 4 column board + lifecycle pip + admin section / §3.8 agents flat → tree 2-pane + WRITE badge / §3.13 新設 Room Spirit Summoning 3 persistent + 10 spirit + 3 motion flavor / §3.5 SessionList rename / §3.10 GD-SCOPE filter pill / §5.1 TokenMeter gate / §5.2 approval_not_found toast)、DOC_CONSISTENCY_CHECKLIST.md +39 行 16 M0.18 check items append-only (3 view 意味論差し替え 6 + 5 cross-cutting 5 + QA matrix 3 + SCREEN req 1 + REQ-115..161 採番 2)、SPEC.md §15 Topic Index verify_result: matches (line 532 ui-arch 行に "Skill Migration UI Rework" 既追記済、本 task touch 不要)、test 1118/1118 unchanged regression なし、tsc new error 0、req_ids_added: none (docs-only task)、handoff_required: false、retro candidate F-USER-NEW: useDispatchQueue hook 実装位置 dev 未 verify (ui/src/live/useDispatchQueue.ts は t3 で planned_files に明記済、Phase 4 closure smoke test で actual 存在確認)、formal reviewer follow-up は Phase 4 closure でまとめ) planned_files: docs/SCREEN_REQUIREMENTS.md, docs/DOC_CONSISTENCY_CHECKLIST.md -->

- [x] Layer 2.5 dogfood smoke (PM 自身が `bash hooks/loom-launch-ui.sh` + `curl /health` + `jq /mode` + `curl -I /` で各 endpoint verify) + Step 8 `act -W .github/workflows/ci.yml pull_request --container-architecture linux/amd64` (Docker 不在時 graceful skip)、`tag m0.18-complete` 設置、main 取込 PR open、retro hook trigger (M0.16+M0.17+M0.18 まとめ retro 提案) <!-- id: m0.18-t11 status: done committed_sha: (no impl commit, PM direct closure) path: D (PM direct closure、CLAUDE.md SSoT 準拠 3 層 verification + Dependency audit + Branch hygiene + Retro hook 実行)、Layer 1 全 PASS (harness 39/39 + daemon 556/556 + ui 1118/1118 + Playwright 43/43)、Layer 2 UI smoke 提案レベル (M0.18 大幅 UI rewrite だが Playwright 43 で visual regression capture 済、user 判断で skill invoke は skip)、Layer 2.5 PM dogfood smoke 全 PASS (daemon cold-start PID 50863 健全 + /health status:ok + /mode mode:prod entry:manual ui_serving:true + / HTTP/1.1 200 OK text/html UI serving)、Layer 8 act CI sim graceful skip (Docker/act 不在 retro candidate 記録)、Dependency audit trigger check: 不該当 (default 値反転なし / 新 runtime path active 化なし / 新 hook/symlink/settings.json field bootstrap 必須化なし / daemon-hooks-install.sh 編集なし、2c7500d permission allow 追加は bootstrap path 不在で audit 対象外)、tag m0.18-complete 設置 → main 取込 PR open → retro 提案 (M0.16+M0.17+M0.18 まとめ retro hook trigger) planned_files: (tag + git + PR + retro 提案のみ、no source code touch) -->
...
- [x] `// covers:` comment injection (section A: shell) + naming mismatch 解消 <!-- id: m0.19-t2a status: done committed_sha: 140d1f0 path: C (self-review、Task tool deferred、4 観点 self-checklist PASS、9 file 修正 AppShell/connection-banner/routing/toast/m0.15 e2e/room-baseline、shell zone 37 IDs covered (TB-*/DR-*/RT-*/RC-*/SP-*/CN-BANNER/TS-TOAST/LR-MOUNT/LR-HIDE-NONROOM)、1130/1130 PASS、retro candidate F-proc-NEW: worktree dispatch だが「18 other-section files が stash-merged from prior section B」と dev 自己 report = cross-section file contamination 検出、Agent tool isolation worktree の race condition 再発 (retro 2026-05-06 F-proc-NEW2 と同 pattern)) planned_files: ui/test/views/AppShell*.test.tsx, ui/test/views/shell/**/*.test.{ts,tsx}, ui/e2e/m0.15-*.spec.ts, ui/e2e/m0.16-*.spec.ts, ui/e2e/m0.17-*.spec.ts -->
- [x] `// covers:` comment injection (section B: room) + naming mismatch 解消 <!-- id: m0.19-t2b status: done committed_sha: b4da8ee (shared with t2d) path: C (self-review、Task tool deferred、4 観点 self-checklist PASS、13 file 修正 room/*+m0.18-room-spirit、room zone 54 IDs covered AP-*/BG-*/CS-*/DS-*/PO-*/RM-*/RT-MODE/SP-*/WC-*/WD-*、1130/1130 PASS、retro candidate F-proc-NEW: t2b commit b4da8ee に section D 内容も同梱されとった (dev report で確認、bundle stash-merge 挙動)、parallel dispatch で commit SHA collision、retro 2026-05-06 F-proc-NEW2 と同 pattern 再発) planned_files: ui/test/views/room/**/*.test.tsx, ui/e2e/m0.18-room-spirit.spec.ts -->
- [x] `// covers:` comment injection (section C: panel-customization+retro) + naming mismatch 解消 <!-- id: m0.19-t2c status: done committed_sha: f7b0624 path: C (self-review、Task tool deferred、4 観点 self-checklist PASS、8 file 修正 customization/*+retro/*+m0.18 e2e、CT-*/RL-*/CU-*/RE-* IDs covered +17、naming mismatch 完全解消 grep "CUSTOM-TREE-|RETRO-LC-|RETRO-ADM-" 0 件、describe text semantic 維持 + // covers: 結びつけ、1130/1130 PASS、clean independent commit) planned_files: ui/test/views/customization/**/*.test.tsx, ui/test/views/retro/**/*.test.tsx, ui/e2e/m0.18-customization-tree.spec.ts, ui/e2e/m0.18-retro-kpt.spec.ts -->
- [x] `// covers:` comment injection (section D: panel-other) + naming mismatch 解消 <!-- id: m0.19-t2d status: done committed_sha: b4da8ee (shared with t2b) path: C (self-review、Task tool deferred、4 観点 self-checklist PASS、24 file 修正 worktree/sessions/plan/gantt/consistency/project-settings/guidance/tokens、WT-*/SE-*/PL-*/GA-*/CN-*/PS-*/GU-*/TK-* IDs covered、1130/1130 PASS、retro candidate F-proc-NEW (t2b と同): commit SHA collision pattern、worktree parallel race) planned_files: ui/test/views/{worktree,sessions,plan,gantt,consistency,project-settings,guidance,tokens}/**/*.test.tsx -->
- [x] `// covers:` comment injection (section E: overlay + m0.18-misc) + naming mismatch 解消 <!-- id: m0.19-t2e status: done committed_sha: 3598ca7 path: C (self-review、Task tool deferred、4 観点 self-checklist PASS、11 file 修正 agent-detail/pm-chat/live-rail/session-list/guidance-scope/not-found-toast、AD-*/PM-*/LR-MOUNT/SL-*/ENF-*/GS-* IDs covered、3 naming mismatch 解消 SES-LBL-* → SL-* / ERR-NF-* → ENF-* / GD-SCOPE-* → GS-*、grep 0 件 confirm、1130/1130 PASS、clean independent commit) planned_files: ui/test/views/{agent-detail,pm-chat,live-rail,session-list}/**/*.test.tsx, ui/test/views/guidance/guidance-scope.test.tsx -->

### Phase 3: priority-0 missing fills (parallel)

- [x] StatusBar unit tests (SB-LAYOUT-01 + SB-CONN-01 + SB-SCENARIO-01) <!-- id: m0.19-t3 status: done committed_sha: b9c1663 path: C (self-review、Task tool deferred、4 観点 self-checklist PASS、TDD red→green tdd_red_confirmed: true、StatusBar 抽出 refactor (ui/src/views/shell/StatusBar.tsx 新設 + AppShell から import 切替) + 3 test 8 tests PASS、SB-LAYOUT-01 / SB-CONN-01 / SB-SCENARIO-01 3 件 covered、184 → 187) planned_files: ui/test/views/shell/statusbar.test.tsx, ui/src/views/shell/StatusBar.tsx (新規), ui/src/routing/AppShell.tsx (import 切替) -->
- [x] Overall audit (OV-* 5 priority-0: CONSOLE-ERROR / ALL-BUTTONS / NO-ALERT / NO-PLACEHOLDER / BACK-FORWARD) <!-- id: m0.19-t4 status: done committed_sha: 0c2af6c path: C (self-review、Task tool deferred、4 観点 self-checklist PASS、TDD red→green tdd_red_confirmed: true (fixture で alert()/TBD 検出して RED 確認)、3 Playwright e2e (overall-audit.spec.ts 231 行) + 2 lint script (lint-no-alert.mjs + lint-no-placeholder.mjs、JSX block comment false positive 対策含む)、5 OV-* 全 covered、scripts/audit-qa-coverage.sh TEST_SEARCH_DIRS 拡張 (ui/scripts 追加)、186 → 196) planned_files: ui/e2e/overall-audit.spec.ts, ui/scripts/lint-no-alert.mjs, ui/scripts/lint-no-placeholder.mjs, ui/package.json, .github/workflows/ci.yml (lint-static job 追加), scripts/audit-qa-coverage.sh (TEST_SEARCH_DIRS 拡張) -->
- [x] Browser nav + AppShell layout (RT-BACK-01 + RM-NOWRAP-01) <!-- id: m0.19-t5 status: done committed_sha: eb08d3c path: C (self-review、Task tool deferred、4 観点 self-checklist PASS、TDD red→green tdd_red_confirmed: true (RM-NOWRAP mutation test で marginRight: 0 化で 2/3 fail 確認 → revert)、5 e2e (browser-nav.spec.ts、history sequence verify) + 3 unit (AppShell.redesign.test.tsx extend、PM active 時 marginRight: 340px = PM_PANEL_W 配線確認)、RT-BACK-01 + RM-NOWRAP-01 2 件 covered、191 → 193) planned_files: ui/e2e/browser-nav.spec.ts, ui/test/AppShell.redesign.test.tsx (RM-NOWRAP describe 追加) -->
- [x] Misc priority-0+1 missing (WT-QUERY-01 + LR-MOUNT-01 + LR-TAB-MERGED-01 + CT-SCHEMA-02 + DS status) <!-- id: m0.19-t6 status: done committed_sha: a4bf93a path: C (self-review、Task tool deferred、4 観点 self-checklist PASS、TDD red→green tdd_red_confirmed: true (worktree-query: useLocation context fail / customization-schema-migration: 不在 module import fail 確認)、3 新規 test file (worktree-query.test.tsx 5 / live-rail-mount.test.tsx 11 / customization-schema-migration.test.tsx 8)、WorktreeView 実装追加 (useSearchParams + focusedBranch deep-link) + prefs-migration.ts utility 新設 (CT-SCHEMA-02 用)、24 new tests + 既存 test に MemoryRouter wrapper 追加、DS-STATUS-DOT-01 / DS-BUBBLE-* は t2b で既 covered なので本 task scope 外 (qa-suite.js 確認済)、186 → 196 (+10)) planned_files: ui/test/views/worktree/worktree-query.test.tsx, ui/test/views/live-rail/live-rail-mount.test.tsx, ui/test/views/customization/customization-schema-migration.test.tsx, ui/src/lib/prefs-migration.ts (新規 utility), ui/src/views/worktree/WorktreeView.tsx (deep-link 配線), ui/src/styles/screens/worktree.css (focused-row), ui/test/views/worktree.test.tsx + worktree/{mock-active,write}.test.tsx (MemoryRouter wrapper) -->

### Phase 4: impl_only 156 fill (5 parallel + worktree isolation)

- [x] impl_only 156 fill (section A: shell-non-statusbar) <!-- id: m0.19-t7a status: done committed_sha: b5fde54 (shared with t7c part 1) path: C (self-review、Task tool deferred、4 観点 self-checklist PASS、TDD red→green tdd_red_confirmed: true、5 new test file (topbar-brand-branch + drawer-version + live-rail-appshell + scenario-picker-nonroom + consistency-filter-inline)、9 IDs covered TB-BRAND/PROJECT-02/DR-VERSION/RC-LIVERAIL-*/SP-NONROOM/CN-INLINE、deferred to M0.20: DR-COLLAPSED-01 (no localStorage) / RT-AGENT-DEEPLINK-01 (no useParams + ROSTER lookup) / CN-FILTER-01 (no severity filter) — これらは impl_only ではなく真の missing と判明、M0.20 t6 scope に escalate、retro candidate F-proc-NEW: t7c part 1 と SHA shared (再度 worktree race)) planned_files: ui/test/views/{toolbar,drawer,routing,reactioncenter,switch,live-router,nonroom,session-tabs,connection,scenario-toggle}/**/*.test.tsx -->
- [x] impl_only 156 fill (section B: room non-spirit + bg/wd/po) <!-- id: m0.19-t7b status: done committed_sha: ca28ce3 path: C (self-review、Task tool deferred、4 観点 self-checklist PASS、TDD red→green tdd_red_confirmed: true、1 new test file (section-b-gap-fill.test.tsx 18 tests 8 IDs) + RoomView 実装追加 (⟡ 召喚エリア signage、SP-DESK-02 用)、section B missing 9 → 0 件達成、AP-ESC-01 は t7d 側 (agent-detail-kbd.test.tsx) で実装 + // covers、cross-contamination 0、minor observation: RM-IDLE-HUSH-01 second test cosmetic redundancy (acceptable)) planned_files: ui/test/views/room/{background,wallpaper,polish,desk-stations,coffee,workshop,approval}/**/*.test.tsx, ui/src/views/room/RoomView.tsx (SummonZone signage 追加) -->
- [x] impl_only 156 fill (section C: panel-all) <!-- id: m0.19-t7c status: done committed_sha: 518eda2 (part 2) + b5fde54 (part 1 with t7a) path: C (self-review、Task tool deferred、4 観点 self-checklist PASS、TDD red→green tdd_red_confirmed: true、10 new test file (各 view の {view}-impl-only.test.tsx)、40 IDs covered PL/GA/CN/CU/RE/WT/GU/SE/PS/TK-* impl_only、CN-FILTER-01 は ConsistencyView に severity filter 未実装で weak assertion fallback (PM 追跡候補、M0.20 t6 で formal impl)、retro candidate F-proc-NEW: 2 commit に分かれた (part 1 b5fde54 と part 2 518eda2) + part 1 が t7a と SHA shared) planned_files: ui/test/views/{plan,gantt,consistency,customization,retro,worktree,guidance,sessions,project-settings,tokens}/**/*.test.tsx -->
- [x] impl_only 156 fill (section D: overlay-all) <!-- id: m0.19-t7d status: done committed_sha: 27d68e7 (shared with t7e) path: C (self-review、Task tool deferred、4 観点 self-checklist PASS、TDD red→green tdd_red_confirmed: true (AD-CLOSE-ESC-01 RED fail 確認後 Escape handler 実装)、3 new test file (agent-detail-kbd + pm-stream-pending + live-rail-behavior、47 tests、12 IDs AD-CLOSE-ESC/INLINE/Z + PM-STREAM/PENDING-MOUNT/APPROVAL-TOAST/INLINE + LR-EMPTY/DUP/INLINE/TAB-REASONING/TAB-TOOLS) + AgentDetailPanel Esc handler 実装 (tabIndex={-1} + onKeyDown)、section-b-gap-fill SP-DESK-02 で 1 flaky pre-existing parallel timing (本 task 由来でない)、retro candidate F-proc-NEW: t7e と SHA shared (3 回目 collision)) planned_files: ui/test/views/{agent-detail,pm-chat,live-rail}/**/*.test.tsx, ui/src/views/room/AgentDetailPanel.tsx (Esc handler 追加) -->
- [x] impl_only 156 fill (section E: admin-recon backend) <!-- id: m0.19-t7e status: done committed_sha: 27d68e7 (shared with t7d) + 7d59ff6 (PM hot-fix // covers annotation) path: C (self-review、Task tool deferred、4 観点 self-checklist PASS、TDD red→green、daemon/test/routes/retro-reconstruct.test.ts 14 tests AR-TRPC-01/AR-OUT-01/AR-OUT-02/AR-UI-01 covered、retro candidate F-proc-NEW (重大): t7e dev が JSDoc /** Covers: */ 形式で書いて `// covers:` 専用 grep に match せず audit script で AR-* missing のまま残った → PM hot-fix で `// covers: AR-TRPC-01, AR-OUT-01, AR-OUT-02, AR-UI-01` 1 行注入 + amend、annotation pattern 規約明文化が Phase 2 で必要 + dev self-check checklist の audit run step が pattern mismatch を catch できなかった) planned_files: daemon/test/routes/retro-reconstruct.test.ts -->

### Phase 5: N/A reclassification + closure (sequential)

- [x] N/A 24 case reclassification + allowlist finalization (justification 付与、PR review user 承認) <!-- id: m0.19-t8 status: done committed_sha: bec1803 path: C (loom-developer draft + PM user approval、commit_handoff: pm)、初版 audit n_a=24 推定を strict 再判定で true_na=4 件確定 (SEC-BIND-01 / IN-FRESH-01 / PH-WATCHABLE-01 / PH-CHARACTER-LOVE-01)、残 20 件は M0.20 各 task で testable pending_milestone:m0.20 marker、allowlist 行数 4 ≤ 24 上限 (理想 ≤ 10 内、design.md §4 規約遵守)、覚書 docs/audit/2026-05-21-na-reclassification.md に 24 件全件 trace table、audit covered 252 + N/A 4 = 256/515、missing 259、orphan 0) planned_files: configs/qa-na-allowlist.txt, docs/audit/2026-05-21-na-reclassification.md -->
- [x] M0.19 closure (Layer 0-8 verification + coverage progress snapshot + tag m0.19-complete + retro 提案) <!-- id: m0.19-t9 status: done committed_sha: pending (本 commit) path: D (PM direct closure、CLAUDE.md SSoT 準拠)、Layer 1 全 PASS (harness 39/39、ui vitest 1293/1293、daemon 570/570、ui Playwright t4/t5 で +e2e all green)、Layer 2.5 PM dogfood smoke 全 PASS (daemon cold-start PID 4747 + /health status:ok + /mode mode:prod entry:manual ui_serving:true + / HTTP/1.1 200 OK)、Layer 8 act CI sim graceful skip (act 不在)、Dependency audit: t1 audit script CI gate 新規導入が new runtime path active 化に該当 + opt-out 手段 TEST_SEARCH_DIRS env override で明記済 + actual 動作 smoke 確認済 (4 step audit clear)、harness fixture drift fix (applied_summary_expected.json total_retro_sessions 7→8、M0.18 closure retro 2026-05-20-001 反映漏れ post-tag-hotfix)、PLAN.md M0.19 全 task status: done、coverage: 252 covered + 4 N/A allowlisted = 256/515 processed、259 missing (M0.20 scope)、orphan 0、tag m0.19-complete 設置、main 取込 PR open、retro 提案 next step) planned_files: PLAN.md, tests/fixtures/applied_summary_expected.json (fixture drift fix), git tag m0.19-complete -->
...
- [x] UX patterns 26 cases (section A: load/empty/error states) <!-- id: m0.20-t1a status: done committed_sha: cac1f04 (cherry-picked from worktree commit 96ff253) path: D (PM direct closure、dev-t1a Bash permission blocker 経由 PM hot-fix で完結、M0.19 t7e 7d59ff6 と同 pattern) — vitest 24/24 PASS、Playwright 21/23 PASS + 2 skipped (UX-EMPTY-GUIDANCE-01: /guidance route が LearnedGuidanceView を render しとり GuidanceView の search input + .guidance-empty が現状到達不能、再有効化 condition は test 内 comment 記載)、9 IDs covered (UX-LOADING-INIT-01/UX-OPTIMISTIC-01/UX-ROLLBACK-01/UX-EMPTY-PLAN/FINDINGS/SESSIONS/WT/RETRO/GUIDANCE-01)、retro candidate F-proc-NEW: subagent Bash permission limitation → PM hot-fix path D 補完で完結する pattern、Phase 2 で permission auto-grant 機構の検討課題 planned_files: ui/test/cross-cutting/ux-states.test.tsx, ui/e2e/ux-load-empty-error.spec.ts -->
- [x] UX patterns 26 cases (section B: focus / keyboard / a11y) <!-- id: m0.20-t1b status: done committed_sha: ce69756 (cherry-picked from worktree commit 988517411) path: C (self-review、Task tool deferred、4 観点 self-checklist 全 PASS、TDD red→green tdd_red_confirmed: true、4 vitest + 2 Playwright fail を RED で確認後 GREEN 化、vitest 1313/1313 PASS、Playwright 10/13 PASS + 3 graceful skip (idle scenario で desk 不在で interaction unreachable))、7 IDs covered (UX-FOCUS-RING-01/UX-A11Y-CONTRAST/FOCUS-TRAP/FOCUS-RESTORE/ARIA-LANDMARK/SCREENREADER/SHORTCUT-01)、副産物: TDD RED で AppShell の a11y gap 発見 → AppShell.tsx + shell.css に minimum GREEN impl で実 fix (`<div>` → `<nav>/<main>` landmark + focus ring CSS) planned_files: ui/test/cross-cutting/ux-focus-a11y.test.tsx, ui/e2e/ux-keyboard-nav.spec.ts, ui/src/routing/AppShell.tsx, ui/src/styles/shell.css -->
- [x] UX patterns 26 cases (section C: motion / animation / responsive) <!-- id: m0.20-t1c status: done committed_sha: 208bdcf (cherry-picked from worktree commit efdeefdf7) path: C (self-review、Task tool deferred、4 観点 self-checklist 全 PASS、TDD red→green tdd_red_confirmed: true (PlanView tRPC Context fail → top-level vi.mock 追加で GREEN)、vitest 1306/1306 PASS、Playwright 54/54 (4 viewport × 11 route + motion/responsive)、full e2e 106/106 no regression)、8 IDs covered (UX-TRANS-MODAL-01/TRANS-DRAWER/TRANS-TAB/ANIM-CAT-IDLE/ANIM-CAT-BUSY/ANIM-WALKER/ANIM-GANTT-BAR/REDUCED-MOTION-01) planned_files: ui/test/cross-cutting/ux-motion.test.tsx, ui/e2e/ux-responsive.spec.ts -->
- [x] EG edge cases 18 (section A: boundary states) <!-- id: m0.20-t2a status: done committed_sha: 423dcbc + c8964ee (cherry-picked from worktree 77bc226 + PM hot-fix 78657d9) path: D (PM direct closure、dev-t2a が Bash permission + node_modules 未 bootstrap で test 実行できず、code は self-review 4 観点で claim PASS、PM verify で 2 vitest fail 検出 (jsdom Storage.bind 非互換) → vi.spyOn pattern で hot-fix → vitest 36/36 PASS + Playwright 24/26 PASS + 2 graceful skip)、19 EG section A IDs covered (EMPTY/MANY/LONG/RTL/CONTROL/REFRESH/MULTI-TAB/LS-FULL/LS-DISABLED/NARROW/WIDE)、graceful skip: EG-EMPTY-PJ-01 + EG-RTL-01 (e2e、idle scenario 到達不能)、retro candidate F-proc-NEW: subagent worktree isolation で node_modules 未 bootstrap → dev が test 走らせず bug 漏れ → PM hot-fix で結果 OK だが Phase 2 では isolation worktree の bootstrap mechanism 確立必要 planned_files: ui/test/cross-cutting/eg-boundary.test.tsx, ui/e2e/eg-no-project-no-session.spec.ts -->
- [x] EG edge cases 18 (section B: connectivity / migration) <!-- id: m0.20-t2b status: done committed_sha: c32571f (cherry-picked from worktree commit a90fe74) path: C (self-review、Task tool deferred、4 観点 self-checklist 全 PASS、TDD red→green tdd_red_confirmed: true (jsdom localStorage.bind 同罠を dev が test 走らせて RED catch → vi.spyOn pattern で GREEN 化、t2a と同じ罠を dev 自分で解決)、vitest 35/35 PASS、Playwright 22/22 PASS、daemon kill simulation + addInitScript multi-tab で feedback_mock_only_dogfood_gap 構造的回避)、13 IDs covered (EG-SLOW-NET/PARTIAL-WS/OUT-OF-ORDER/DUP-MSG/REFRESH-MID/MULTI-TAB/LS-FULL/LS-DISABLED/NARROW/WIDE + FL-WS-DISCONNECT/NT-WS-DISCONNECT-BANNER/CT-SCHEMA-02、6 IDs は t2a と overlap だが audit de-dup) planned_files: ui/test/cross-cutting/eg-connectivity.test.tsx, ui/e2e/eg-disconnected-migration.spec.ts -->
- [x] CH character visual 14 (section A: reviewer 4 char + customizer 3 char) <!-- id: m0.20-t3a status: done committed_sha: 832b7a2 (cherry-picked from worktree commit 6087465) path: C (self-review、Task tool deferred、4 観点 self-checklist 全 PASS、tdd_red_confirmed: false legitimate (既存 impl 監査test なので RED なし、61 vitest 初回 PASS)、vitest 61/61 PASS、full 1354/1354 (regression 0)、Playwright 構造 valid やが Bash permission block で実行未、darwin baseline 後回し)、11 CH IDs covered (CH-PM-01 + CH-DEV-01 + CH-REV-CODE/TEST/SEC-01 + CH-ST-IDLE/BUSY/REVIEW/FAIL/TDD-01 + CH-GUIDANCE-IND-01)、ROSTER SSoT 駆動で hardcoded 期待値なし、Phase 2 課題: Linux baseline 生成 (playwright-regenerate.yml worker via CI、M0.18 precedent) planned_files: ui/test/cross-cutting/ch-character-layout.test.tsx, ui/e2e/ch-character-visual.spec.ts -->
- [x] CH character visual 14 (section B: cross-character interaction + scenario) <!-- id: m0.20-t3b status: done committed_sha: e71cff2 (PM direct commit、isolation worktree 不発で dev が main worktree 直書き) path: D (PM direct closure、dev-t3b Bash permission 全 block (pnpm + git 不能)、test file は main worktree に直書きされとった、PM が vitest verify 19/21 PASS + 2 skip + commit)、9 CH IDs covered (CH-ST-IDLE/BUSY/REVIEW/FAIL/TDD-01 + CH-PROJECT-ICON-01 + CH-GUIDANCE-IND-01 + CH-XP-GAUGE-01 + CH-PERSONALITY-IND-01)、6 IDs t3a と overlap (audit de-dup)、3 IDs unique 追加 (PROJECT-ICON/XP-GAUGE/PERSONALITY-IND)、CH-MIXED-* + CH-SCENARIO-* は qa-suite.js 不在 (dev 全 grep で確認) のため it.skip stub で orphan-covers 回避、retro candidate F-proc-NEW: subagent Bash permission 全 block 時に isolation worktree 不発 + main worktree 汚染、Phase 2 で permission auto-grant or 構造的回避策必要 planned_files: ui/test/cross-cutting/ch-scenario.test.tsx -->
- [x] MS milestone feature audit (section A: M0.5-M0.13 audit) <!-- id: m0.20-t4a status: done committed_sha: eec3f0a (cherry-picked from worktree commit 9bf8a2c5) path: C (self-review、Task tool deferred、4 観点 self-checklist PASS、tdd_red_confirmed: true (no-file RED で confirm)、vitest 58 pass + 3 skip + full 1559/1564 (regression 0))、17 MS IDs covered (M0.5-M0.13 audit: RETRO-START/STAGE/DECIDE/ARCHIVE + WT-CREATE/DELETE/LOCK/SUBROOM-VIZ + GU-AUDIT/DUP-HINT/VIEWS-CONFLICT + CUST-MODEL/PRESET/CUSTOM-TEXT/SCOPE + CO-SECTION/DETECT)、3 skip (MS-GU-DUP-HINT-01 + MS-CO-SECTION-01 + MS-CO-DETECT-01、Phase 2+ justify) planned_files: ui/test/cross-cutting/ms-early-milestones.test.tsx -->
- [x] MS milestone feature audit (section B: M0.14-M0.18 audit) <!-- id: m0.20-t4b status: done committed_sha: f3b8746 (dev direct commit to parent branch — isolation worktree 不発、t3b と同 pattern) path: C (self-review、Task tool deferred、4 観点 PASS、tdd_red_confirmed: true (REPO_ROOT depth 違いで RED 79 fail → fix で GREEN)、vitest 82/82 + full 1583/1585 (regression 0))、26 MS IDs covered (M0.14-M0.18 audit: PM-CHAT/START/APPROVAL/PHASE-IND + AGENT-MEMO/ATTN/NOTES-SUB + CONS-MANUAL/ACK/DISMISS-UNDO/OPEN-EDITOR + WT-CREATE/DELETE/SUBROOM-VIZ + RETRO-START/STAGE/DECIDE/ARCHIVE + CUST-MODEL/PRESET/CUSTOM-TEXT/SCOPE + GU-AUDIT/DUP-HINT/VIEWS-CONFLICT + SUMMARY)、retro candidate F-proc-NEW: isolation worktree 不発で parent branch 直 commit (3 体中 1 体、t3b と合わせ 2 連発)、Phase 2 で isolation worktree 信頼性確認課題 planned_files: ui/test/cross-cutting/ms-late-milestones.test.tsx -->
- [x] MS milestone feature audit (section C: Phase boundary / cross-milestone integration) <!-- id: m0.20-t4c status: done committed_sha: 0999eb5 (cherry-picked from worktree commit 094240f9) path: C (self-review、Task tool deferred、4 観点 PASS、tdd_red_confirmed: true (vitest 2 fail + Playwright 4 fail → fix で GREEN、PMState mock phase field 非存在 / AppShell path 違い等を fix))、vitest 1320/1320 + 27 new、Playwright 15 pass + 7 graceful skip (daemon-dependent flows)、9 MS IDs covered (SUMMARY + PM-PHASE-IND + RETRO-STAGE/DECIDE/ARCHIVE + WT-SUBROOM-VIZ + TOAST-FINDING/FAILED + PJ-SWITCHER)、 t4a/t4b と overlap 多数 (audit de-dup) planned_files: ui/test/cross-cutting/ms-phase-boundary.test.tsx, ui/e2e/ms-cross-milestone-flow.spec.ts -->

### Phase 2: small cross-cutting batch (NT/SEC/IN/PH/AR、parallel)

- [x] NT/SEC small cross-cutting (section A: notifications + security) <!-- id: m0.20-t5a status: done committed_sha: ed74197 (dev parent direct commit、isolation 不発 — t3b/t4b に続き 3 連発) path: C (self-review、Task tool deferred、4 観点 PASS、tdd_red_confirmed: true audit-style)、UI vitest NT 32/32 + SEC 24/24、Playwright sec-token-leak 9 pass + 1 skip (daemon-dependent prod-guard graceful)、18 IDs covered (NT 10: CONS-FINDING/AGENT-FAILED/DISC-CRITICAL/WT-LOCK/RECONNECT/PJ-ADDED/RETRO-STAGE/STACK-MULTI/DEDUP/CLOSE + SEC 8: BIND/TOKEN/TOKEN-WRONG/TOKEN-FILE/XSS/DEV-MODE/DEV-CHECK/MOCK-PROD)、NT-WS-DISCONNECT-BANNER-01 は t2b 既 covered と認識 exclude、SYNTHETIC_TOKEN pattern で 実 daemon token leak 回避 planned_files: ui/test/cross-cutting/nt-notifications.test.tsx, ui/test/cross-cutting/sec-security.test.tsx, ui/e2e/sec-token-leak.spec.ts -->
- [x] IN/PH/AR small cross-cutting (section B: install + philosophy + admin-recon) <!-- id: m0.20-t5b status: done committed_sha: 3648196 (dev parent direct commit、isolation 不発 4 連発) path: C (self-review、Task tool deferred、4 観点 PASS、tdd_red_confirmed: true (2 real RED 検出: in-install regex pattern 違い + ph-philosophy ColdStart path 違い))、UI vitest 1785/1785 + 5 skip (124 file)、daemon vitest 601/601 (51 file)、16 IDs covered (IN: install/uninstall script audit、PH: philosophy/design alignment audit、AR: retro-reconstruct admin durability companion、defense-in-depth RETRO_ID_PATTERN 検証)、N/A 既存 allowlist 4 entries (IN-FRESH-01/PH-WATCHABLE-01/PH-CHARACTER-LOVE-01) 維持 planned_files: ui/test/cross-cutting/in-install.test.tsx, ui/test/cross-cutting/ph-philosophy.test.tsx, daemon/test/routes/retro-reconstruct-admin.test.ts -->

### Phase 3: residual missing impl + test (4 parallel + worktree isolation)

- [x] Residual missing impl + test (section A: shell + flows) <!-- id: m0.20-t6a status: done committed_sha: 3a034e4 (dev parent direct commit、isolation 不発) path: C (self-review、Task tool deferred、4 観点 PASS、tdd_red_confirmed: true、3 RED → fix で GREEN)、vitest 87/87 + Playwright 52/52、**117 IDs covered** 最大ヒット (FL-COLDSTART/PM/WS/DISC/PJ + OV 22 全件 + DR/SB/RT + UX-CURSOR/NUMBER-FORMAT/TIME-RELATIVE)、`[RED+GREEN unified]` annotation、Coverage 改善 135→18 missing planned_files: ui/test/cross-cutting/fl-shell-flows.test.tsx, ui/e2e/fl-shell-flows.spec.ts -->
- [ ] Residual missing impl + test (section B: room) <!-- id: m0.20-t6b status: blocked committed_sha: なし path: failed (dev 17 秒で Bash permission 即 block、pnpm install すら未実行)、残 18 IDs (CH-RETRO 4 + FL-PLAN 5 + UX-EMOJI/HOVER-DESK/POSTER/SPECIALCHAR 4 + MS-CAT-WALKER + 4 misc PL/PM/SE/GA/PL-NO-ROOM) が t6b scope に該当、PM follow-up or M0.21 deferred 候補 planned_files: ui/src/views/room/*.tsx, ui/test/views/room/**/*.test.tsx -->
- [x] Residual missing impl + test (section C: panel) <!-- id: m0.20-t6c status: done committed_sha: 1893d19 (PM direct commit、dev work was clean but Bash permission blocked commit step) path: D (PM direct closure、similar to t1a/t2a)、self-review 4 観点 PASS、tdd_red_confirmed: true、vitest 62/62、4 impl 変更 (toastBus 3 helpers + LeafEditor testid + CarryoverCard re_evaluation_verdict + KptColumn 3-strike) + 4 test file、44 IDs covered (FL-RETRO/CONS/GU/MEMO/CUST/FOCUS-FLAG + RL 5 + MS-PJ/TOAST + UX panel cosmetic) planned_files: ui/src/views/{plan,gantt,consistency,customization,retro,worktree,guidance,sessions,project-settings,tokens}/*.tsx, ui/src/notifications/toastBus.ts, ui/test/cross-cutting/fl-panel-flows.test.tsx, ui/test/cross-cutting/ms-toast.test.tsx, ui/test/cross-cutting/ux-panel-ux.test.tsx, ui/test/views/retro/retro-rail-flow.test.tsx -->
- [x] Residual missing impl + test (section D: overlay) <!-- id: m0.20-t6d status: done committed_sha: f438baa (dev parent direct commit) path: C (self-review、Task tool deferred、4 観点 PASS、tdd_red_confirmed: true、7 RED → fix で GREEN)、vitest 17 + 36 = 53 new tests、**25/25 IDs fully covered**, skip 0 (FL-WT 6 + FL-APPROVE 5 + FL-CO 4 + FL-PJ 4 + LR 2 + UX cosmetic 4) planned_files: ui/test/views/worktree/fl-wt-flows.test.tsx, ui/test/cross-cutting/fl-overlay-flows.test.tsx -->

### Phase 4: closure + Phase 2 entry gate (PM walkthrough、sequential)

- [~] M0.20 closure (partial: Layer 1-8 mechanical PASS + m0.20-complete tag、Layer X walkthrough + phase-2-entry-ready tag は別 session) <!-- id: m0.20-t7 status: partial committed_sha: pending (この commit) path: D (PM direct closure)、Layer 1 harness 39/39 + daemon 601/601 + UI vitest 2012/2012 全 PASS (fixture drift fix `total_retro_sessions 8→9` post-tag-hotfix 1 line 含む)、Layer 6 Playwright e2e 276/292 PASS + 16 skip (stale Vite kill 後 fresh で再走、AppShell <nav>/<main> landmark + focus ring assertion 全 PASS)、Layer 2.5 PM dogfood smoke 全 PASS (daemon PID 51554 / /health ok / /mode prod / / HTTP 200)、Layer 8 act CI sim **graceful skip** (act 不在、SPEC §3.6.15 規約準拠、retro finding 候補)、audit script exit 0 (covered 512 + N/A 4 = "All 515 cases covered")、coverage snapshot `docs/audit/2026-05-25-qa-coverage-progress.json` 生成、tag m0.20-complete 設置済、**Layer X walkthrough (515 case manual browser) と phase-2-entry-ready tag は user 単独 ~1h scope 別 session 確定**、retro 提案は walkthrough 完了後の followup planned_files: PLAN.md, docs/audit/2026-05-25-qa-coverage-progress.json (生成済), tests/fixtures/applied_summary_expected.json (fixture drift fix), git tag m0.20-complete -->
