...
詳細: 未作成（impl phase で `docs/plans/2026-05-02-claude-loom-m3.0-room-view.md` を `loom-write-plan` skill で詳細化）

設計合意（2026-05-02 spec phase 対話、SPEC §3.6.9.1 SSoT）:
- **α-1 確定**: Phaser 3 React 内 mount = 自前 `useEffect` + `useRef`（library 依存ゼロ）
...
- [ ] Phaser 3 React 内 mount（自前 useEffect、game lifecycle 管理 + HMR 対応） <!-- id: m3.0-t1 status: todo -->
- [ ] ピクセルルーム基本タイル（tile map JSON + tokens.css `--color-bg` 連携） <!-- id: m3.0-t2 status: todo -->
- [ ] エージェントスプライト + 状態アニメ（idle/busy/失敗、agent_id ↔ sprite 1:1 mapping） <!-- id: m3.0-t3 status: todo -->
