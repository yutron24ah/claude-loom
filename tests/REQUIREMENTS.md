...
- **REQ-076**: Room 壁掛け 3 poster (GanttPoster / PlanPoster / ConsistencyPoster) のハードコード fixture を撤去し、`useScenario()` 経由のライブデータで scenario 駆動化する。GanttPoster は `scenario.gantt` (GanttData) から ROOM_AGENTS 5 体の行を抽出し、KIND_COLOR で mini-bar を描画、`gantt.nowPct` で now-line を配置、`gantt.windowLabel` をヘッダに表示する。PlanPoster は `scenario.todos` (最大 4 件) + `scenario.milestones` (最大 3 件) + `scenario.todosUpdatedAt` を表示し、TodoStatus に応じたアイコン・MilestoneStatus に応じたバー色 (STATUS_COLOR) を付与する。ConsistencyPoster は `scenario.findings` 上位 4 件を表示し、`sev=high && status=open` の件数で NEW バッジを動的に計算する。`ui/test/views/room/wall-posters/posters-mock-active.test.tsx` の 12 ケース全 pass (GanttPoster: 5 row render / now-line at nowPct / windowLabel / bar segments、PlanPoster: 4 todo render / status icons / 3 milestones / todosUpdatedAt、ConsistencyPoster: top 4 findings / NEW 2 badge / finding ids visible)。`ui/test/views/room/wall-posters.test.tsx` (37 tests、useScenario mock に更新) + `ui/test/views/room/room-mock-active.test.tsx` (4 tests、gantt/todos/findings フィールド追加) regression なし。`pnpm --filter @claude-loom/ui test` 全 pass (967 tests)。**rationale**: M0.15 Phase 4 t15 — M0.11.4 ハードコード fixture を撤去し、redesign bundle の useScenario 経由データで Room wall posters を scenario-driven に置換する。redesign/screens/room.jsx L103-199 の GanttPoster / PlanPoster / ConsistencyPoster 視覚 SSoT に準拠。
=======
## M0.15 Phase 4 t14: AppShell redesign 全面移植

- **REQ-075**: `ui/src/routing/AppShell.tsx` が redesign/Redesign App.html の shell HTML/CSS を TypeScript/JSX で全面移植し、TopBar (brand + project switcher + 4 discipline metrics + conn status + drawer toggle) / Drawer (168px → 40px 可変、11 nav links × 3 groups: OPERATE/MANAGE/SETTINGS) / StatusBar (scenario label + events + project path) / ScenarioPicker (idle/active/failed/live switcher) を実装する。`useScenario()` を data source として project / conn / disciplineMetrics / label をリアクティブに描画する。TopBar の 4 metrics は `data-testid="metric-parallel|metric-task-tool|metric-tdd-order|metric-verdict"`。Drawer の nav links は `data-testid="nav-link-{id}"` (11 個)、3 group は `data-testid="drawer-group-{operate|manage|settings}"`。collapse toggle は topbar-drawer-toggle ボタンで drawer の `data-collapsed` 属性が "true" に切替。t13 の PMChatPanel right column mount slot (340px、z-5) は interface 互換で維持。Room canvas (data-testid="room-canvas") は常時 mount、Outlet パネルオーバーレイ routing は維持。`ui/test/AppShell.test.tsx` (19 tests: room-canvas persistence + panel overlay + Escape/click dismiss + TopBar/Drawer 構造) および `ui/test/AppShell.redesign.test.tsx` (19 tests: 4 TopBar + 5 Drawer + 3 StatusBar + 3 collapse + 1 PMChat + 1 ScenarioPicker + 2 nav routing) 合計 38 ケース全 pass。`pnpm --filter @claude-loom/ui test` 922 tests pass (4 pre-existing failures 変化なし)。**rationale**: M0.15 Phase 4 t14 — M0.11.4 ハードコード DisciplineHeader + Sidebar を redesign SSoT のシェル構造 (TopBar / Drawer / StatusBar / ScenarioPicker) に全面置換。ConnectionBanner (→ daemon/zod chain) を TopBar conn dot に置換してテスト分離性を向上。t13 PMChat panel mount interface は完全維持。
>>>>>>> worktree-agent-a8e9656beaf124fea
