...
## M0.15: UI Redesign Port

- **REQ-063**: `ui/src/views/plan/PlanView.tsx` が `useScenario()` (redesign/api/websocket) のみに依存し、usePlanItems / useTodoWrite / usePlanMutations を import しない。左ペイン (data-testid="plan-long-term") が scenario.milestones をマイルストーンカード (`plan-milestone` testid + `milestone-progress-bar` + `milestone-progress-fill` + `milestone-child`) で表示し、右ペイン (data-testid="plan-short-term") が scenario.todos を todo-item (data-status 属性付き) + scenario.todosUpdatedAt (`todos-updated-at` testid) で表示する。タブ (active / 完了 archive / 編集) の切替で左ペイン表示が切替わる。`ui/test/views/plan/plan-mock-active.test.tsx` の 4 ケース全 pass: todos 4件 status glyph / milestones 2件 progress bar / milestone children 3件 nested / todosUpdatedAt timestamp。`pnpm --filter @claude-loom/ui test` 全 pass (regression なし)。
