...
session_start hook 発火時：
  1. cwd を取得
  2. `git rev-parse --git-common-dir` で repo の正規パスを取る（worktree 対応）
  3. 正規パスのハッシュ = project_id 候補
  4. プロジェクトルートに `.claude-loom/project.json` マーカーあれば最優先
  5. projects テーブルに既存？
     - YES → sessions.project_id にセット
     - NO  → 「未登録」状態の暫定 row を INSERT
              GUI で「新規プロジェクト検出、登録しますか？」表示
              /loom-spec 実行で本登録（marker file 生成）
```
...
- `agents/`, `commands/`, `hooks/`, `skills/` を `~/.claude/<type>/` にシンボリックリンク
- `daemon/` を build → `~/.claude-loom/daemon.js` に配置（or symlink）
- `ui/` を build → `~/.claude-loom/ui/` に配置
- `~/.claude/settings.json` に hooks 登録（jq 書き換え + atomic mv、claude-blog-skill 流儀）
...
- `PLAN.md` — 実装マイルストーン（writing-plans skill で生成予定）
- `docs/SCREEN_REQUIREMENTS.md` — 画面要件詳細
- `docs/EVENT_SCHEMA.md` — hook payload 完全仕様（実装初期に作成）
