...
## verdict_evidence 保存規律（M2.1 から、SSoT は SPEC §3.9.10 + §6.9.5）

retro session 開始時、`loom-retro-pm` agent は **直前 milestone の reviewer dispatch evidence を独立 file に保存** する。「review skip」と「指摘ゼロ pass」を retro 中 / 後の audit で機械的に区別可能にする discipline 機構。M0.13 で codify された旧設計（`pending.json` 内 `verdict_evidence` field）を refactor し、M2.1 から独立 file + zod schema 化に移行。

### 設計概要

- **保存 path**: `<project>/.claude-loom/retro/<retro_id>/verdict_evidence.json`（per-retro-instance file、prefs と分離）
...
- **schema**: SPEC §6.9.5（zod 完全定義、`schema_version: 1`）
- **責務分離**: PM agent は **file write しない**、final report に hint reference を残すのみ
...
### PM hint reference 形式（loom-pm が milestone tag 設置時に final report に残す）

```
[reviewer-dispatch-refs]
- task_id=m2-t5, commit_sha=<sha>, reviewer_agent=loom-reviewer, review_mode=single
- task_id=m2-t6, commit_sha=<sha>, reviewer_agent=loom-reviewer, review_mode=single
...
```

retro-pm の lazy build accuracy 補強用、PM 自身は file write しない（責務分離維持）。

### audit 用途

- **process-axis lens** が「review skip vs 指摘ゼロ pass」を機械的に区別（commit_sha 不在 = review skip / aspect_findings 全 pass + findings_count=0 = 指摘ゼロ pass）
...
M0.11 で「v1 manual prune」と意識的 deferral 決定された learned_guidance lifecycle を M0.11.1 で structural 自動化（SPEC §6.9.4.5）。

**2 mechanism hybrid**:

1. **`ttl_sessions` main (auto-deactivate、決定論的)**:
   - aggregator が retro 終了時、`ttl_sessions != null && retro_session_distance >= ttl_sessions` の guidance を `active: false` 自動化
   - `ttl_sessions: null` (default) は infinite、break compat なし
2. **`last_used_in` audit (proposal、dynamic)**:
   - meta lens が Stage 1 で `last_used_in` 確認、N session 連続未使用 (initial threshold = 3) → "stale guidance" として meta-axis finding 化
   - user 承認後 deactivate（auto じゃない、user 主導）
3. **責務分離**: auto-deactivate = 決定論的 ttl、user 承認 prune = dynamic last_used_in、両 prune 経路明確

**lens 参照時の `last_used_in` update**: 各 retro lens が learned_guidance を Customization Layer 経由で injection した時、aggregator が retro 終了時に該当 guidance の `last_used_in: <current_retro_id>` を update（lens は read のみ、write は aggregator 中央集権）。

### Symptomatic rollback example (P4 理想形 archive)

M3.0 retro 2026-05-02-002 で生まれた proc-NEW-1 (counter-arguer stale check section) は **interim safety net** として approve、M0.11.1 closure 時に **物理削除 (rollback)** される。これは SPEC §3.9.x P4 「症状対処 (symptomatic) を構造的解決 (structural) 後に消滅させる cleanup loop」の最初の実例。pending.json `apply_history` には：

```json
[
  { "commit_sha": "a3f3cee", "applied_at": ..., "apply_type": "immediate", "note": "symptomatic interim safety net" },
  { "commit_sha": "<m0.11.1-rollback>", "applied_at": ..., "apply_type": "rollback", "note": "structural M0.11.1 完成、symptomatic patch 物理削除" }
]
```

`applied_summary.json` の `last_apply_history_entry` は rollback note を含む → 4 lens は「once applied but later rolled back」を機械的に把握、stale re-up を防ぐと同時に「ここに何があったか」も理解できる。
